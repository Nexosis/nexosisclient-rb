...
describe NexosisApi::Client::Datasets do
    describe "#create_dataset_json" do#, :vcr => {:cassette_name => "create_dataset_json"} do
        context "given a dataset json hash" do
            it "returns a dataset summary" do
                datahash = {"columns": {"reading": {"datatype": "numeric","role": "numeric"},"timestamp": {"datatype": "date","role": "date"}}}
                data = JSON.load(File.open('spec/fixtures/sampledata.json'))
                actual  = test_client.create_dataset_json 'TestRuby_Json', datahash
